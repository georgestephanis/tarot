...
add_filter( 'tarot_get_deck', 'tarot_8bit_deck' );
function tarot_8bit_deck( $deck ) {
	$deck8bit = get_deck_8bit();
...
function override_upload_dir_8bit( $uploads ) {
	$uploads['path'] = $uploads['basedir'] . '/tarot/8bit';
	return $uploads;
}

function sideload_8bit_deck() {
	require_once ABSPATH . 'wp-admin/includes/file.php';

	$upload_dir = wp_get_upload_dir();
	$dir_8bit = $upload_dir['basedir'] . '/tarot/8bit';
	if ( ! file_exists( $dir_8bit ) ) {
		wp_mkdir_p( $dir_8bit );
	}

	$deck = get_deck_8bit();

	add_filter( 'upload_dir', 'override_upload_dir_8bit' );
	...
	foreach ( $deck as $card ) {
		...
		wp_handle_sideload( $file_array, array( 'test_form' => false ) );
	}
